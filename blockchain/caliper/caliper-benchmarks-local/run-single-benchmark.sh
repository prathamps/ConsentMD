...
# Configuration. bench-network.yaml is generated by setup/provision-identities.js
# and carries the CA-enrolled identities the chaincode's policy requires; the
# static config is only a fallback for pre-CA setups.
NETWORK_CONFIG="networks/fabric/bench-network.yaml"
if [ ! -f "$NETWORK_CONFIG" ]; then
    echo "Note: $NETWORK_CONFIG not found (run: node setup/provision-identities.js)"
    echo "Falling back to networks/fabric/consent-management-network.yaml"
    NETWORK_CONFIG="networks/fabric/consent-management-network.yaml"
fi
RESULTS_DIR="results/single-$(date +%Y%m%d_%H%M%S)"

# Let the workloads record per-transaction latencies and dataset manifests.
export CONSENTMD_RESULTS_DIR="$PWD/$RESULTS_DIR"
export CONSENTMD_RUN_LABEL="${1:-single}.run1"
