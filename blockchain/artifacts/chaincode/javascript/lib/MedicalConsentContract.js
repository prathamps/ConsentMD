...
class MedicalConsentContract extends Contract {
    ...
    constructor() {
        // Explicitly name the contract. This is a best practice.
    ...
    async registerDoctorProfile(ctx, name, specialization) {
        ...
        const compositeKey = ctx.stub.createCompositeKey('DoctorProfile', [doctorId]);
     ...
     */
    async createPatientRecord(ctx, fileName, s3ObjectKey, details) {
     ...
     */
    async createMedicalRecord(ctx, patientId, recordDetails, fileName, s3ObjectKey) {
        await this._verifyDoctorRole(ctx);
    ...
    async addPrivateNoteToRecord(ctx, collection, recordId) {
        ...
        const privateKey = ctx.stub.createCompositeKey('Record~Note', [recordId, privateNote.noteId]);
        await ctx.stub.putPrivateData(collection, privateKey, Buffer.from(JSON.stringify(privateNote)));
    ...
    async updateRecordDetails(ctx, recordId, newDetails) {
        ...
        if (record.doctorCreatorId !== cid.getID()) {
            throw new Error(`Only the creating doctor (${record.doctorCreatorId}) can update this record.`);
        }
        if (record.archived) {
        ...
        await ctx.stub.putState(record.recordId, Buffer.from(JSON.stringify(record)));
        ctx.stub.setEvent('UpdateMedicalRecord', Buffer.from(JSON.stringify(record)));
        return JSON.stringify(record);
    }
    ...
    async getRecordById(ctx, recordId) {
        ...
        throw new Error(`User ${cid.getID()} is not authorized to access record ${recordId}`);
    }
    ...
    async _verifyDoctorRole(ctx) {
        ...
        if (!cid.assertAttributeValue('role', 'doctor')) {
            throw new Error('Only users with the "doctor" role can perform this operation.');
        }
    }
    ...
    async _verifyPatientOwnership(ctx, patientId) {
        ...
        if (cid.getID() !== patientId) {
            throw new Error(`Only the patient ${patientId} can perform this operation.`);
        }
    }
    ...
    async _verifyAccess(ctx, doctorId, recordId) {
        try {
            ...
            return false; // Not a doctor, no access.
        }
        ...
        return !result.done; // If a result exists, access is granted.
    }
