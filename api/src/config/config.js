...
console.log('DOCKER_MODE from process.env:', process.env.DOCKER_MODE);
const envVarsSchema = Joi.object()
  .keys({
    ...
    BLOCKCHAIN_CHANNEL_NAME: Joi.string().required().description('Blockchain channel name'),
    BLOCKCHAIN_CHAINCODE_NAME: Joi.string().required().description('Blockchain chaincode name'),
    DOCKER_MODE: Joi.boolean().default(false).description('Flag to indicate if running in docker'),
  })
  .unknown();
...
module.exports = {
  ...
  blockchain: {
    channelName: envVars.BLOCKCHAIN_CHANNEL_NAME,
    chaincodeName: envVars.BLOCKCHAIN_CHAINCODE_NAME,
  },
  jwt: {
